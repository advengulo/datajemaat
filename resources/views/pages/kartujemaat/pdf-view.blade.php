    ...
    {{-- <div class="col-md-12"> --}}
        <div class="row">
            ...
            <div class="col-md-6" style="text-align:center">
                <h1 style="margin-left:0.4cm; margin-top:0px; padding-top:0px">KARTU KELUARGA BNKP JEMAAT KOTA
                    GUNUNGSITOLI</h1> <br>
                <h2 style="font-size: 18pt; font-weight:bold; text-align:center; margin-top:5px">NOMOR :
                    {{$nomor_kartu}}</h2>
            </div>
        </div>
        ...
        <div class=row style="clear: both;">
            ...
            <div class="col-md-5" style="height:20px; padding-left:1px;">
                <h5 style="font-size:10pt; margin-top:5px; font-weight:900"><span style="font-weight:normal">:</span>
                    {{$data_jemaat->jemaat_nama}} </h5> <br>
            </div>
            ...
            <div class="col-md-3" style="height:20px;">
                <h5 style="font-size:11px; margin-top:5px; ">: {{$data_jemaat->id_lingkungan}}
                    {{$data_jemaat->lingkungan->nama_lingkungan}}</h5> <br>
            </div>
        </div>
        <div class="row" style="clear: both;">
            <div class="col-md-3" style="height:20px; width: 230px; padding-left:35px;">
        ...
        <table class="tablepdf">
            <tbody>
                ...
                @foreach ($dataKartuKeluargas as $dataKartuKeluarga)
                    <tr>
                        <td style="text-align:center;" @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            {{$i}}</td>
                        <td>
                            @if($dataKartuKeluarga->jemaat_gelar_depan != "" && $dataKartuKeluarga->jemaat_gelar_belakang != ""){{$dataKartuKeluarga->jemaat_gelar_depan}}.{{$dataKartuKeluarga->jemaat_nama}},{{$dataKartuKeluarga->jemaat_gelar_belakang}}
                            @elseif($dataKartuKeluarga->jemaat_gelar_depan != ""){{$dataKartuKeluarga->jemaat_gelar_depan}}.{{$dataKartuKeluarga->jemaat_nama}},@elseif($dataKartuKeluarga->jemaat_gelar_belakang != ""){{$dataKartuKeluarga->jemaat_nama}},{{$dataKartuKeluarga->jemaat_gelar_belakang}}@else{{$dataKartuKeluarga->jemaat_nama}}
                            @endif
                        </td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_status_dikeluarga == 1 || $dataKartuKeluarga->jemaat_status_dikeluarga == 0 && $dataKartuKeluarga->jemaat_kk_status == true)
                                KEPALA KELUARGA
                        ...
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_jenis_kelamin == "l") LAKI-LAKI
                            @else PEREMPUAN
                            @endif
                        </td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            {{$dataKartuKeluarga->jemaat_tempat_lahir}}</td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_tanggal_lahir != null){{$dataKartuKeluarga->jemaat_tanggal_lahir->translatedFormat('d F Y')}}
                            @else -
                            @endif
                        </td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_tanggal_baptis != null){{$dataKartuKeluarga->jemaat_tanggal_baptis->translatedFormat('d F Y')}}
                            @else -
                            @endif
                        </td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_tanggal_sidi != null){{$dataKartuKeluarga->jemaat_tanggal_sidi->translatedFormat('d F Y')}}
                            @else -
                            @endif
                        </td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_status_perkawinan == 1) KAWIN
                            @elseif($dataKartuKeluarga->jemaat_status_perkawinan == 2) BELUM KAWIN
                            @elseif($dataKartuKeluarga->jemaat_status_perkawinan == 3) DUDA
                            @elseif($dataKartuKeluarga->jemaat_status_perkawinan == 4) JANDA @endif
                        </td>
                        <td @if($dataKartuKeluarga->jemaat_nama_alias != null) rowspan="2" @endif>
                            @if($dataKartuKeluarga->jemaat_tanggal_perkawinan != null)
                                {{$dataKartuKeluarga->jemaat_tanggal_perkawinan->translatedFormat('d F Y')}}
                            @else -
                            @endif
                        </td>
                    </tr>
                ...
                @foreach ($dataKartuKeluargas as $dataKartuKeluarga)
                    <tr>
                        ...
                        <td>
                            @if($dataKartuKeluarga->jemaat_tanggal_status != null){{$$dataKartuKeluarga->jemaat_tanggal_status->translatedFormat('d F Y')}}
                            @else -
                            @endif
                        </td>
                        <td>-</td>
        ...
        <div class="row" id="fixed">
            <div class="row" style="font-size:11px;">
                ...
                <div class="col-md-6" style="text-align:center; margin-bottom:55px">Badan Pekerja majelis jemaat kota
                    gunungsitoli </div>

            </div>
            <div class="row" style="font-size:11px; clear:left">
                <div class="col-md-6" style="text-align:center;">
                    @if($data_jemaat->jemaat_gelar_depan != "" && $data_jemaat->jemaat_gelar_belakang != ""){{$data_jemaat->jemaat_gelar_depan}}.{{$data_jemaat->jemaat_nama}},{{$data_jemaat->jemaat_gelar_belakang}}
                    @elseif($data_jemaat->jemaat_gelar_depan != ""){{$data_jemaat->jemaat_gelar_depan}}.{{$data_jemaat->jemaat_nama}},@elseif($data_jemaat->jemaat_gelar_belakang != ""){{$data_jemaat->jemaat_nama}},{{$data_jemaat->jemaat_gelar_belakang}}@else{{$data_jemaat->jemaat_nama}}
                    @endif
                </div>
                <div class="col-md-3" style="text-align:center; text-transform:none">SNK dr. VICTOR K.F. TEL, Sp.B. <br>
                    KETUA</div>
